    ...
    if($check_auth_client == true){    
      ...
      if($this->form_validation->run() == TRUE){
        ...
        if(empty($detailService)){
          $output = ['result' => false, 'message' => 'This service is not available'];
    ...
    if($check_auth_client == true){    
      ...
      if($this->form_validation->run() == TRUE){
        ...
        if(empty($detailEvent)){
          $output = ['result' => false, 'message' => 'This event is not available'];
          $this->response($output, 422); die;
        }
  ...
  function advertisementDetail_get(){
    ...
    if($check_auth_client == true){    
      ...
      if($this->form_validation->run() == TRUE){
        ...
        if(empty($detailAd)){
          $output = ['result' => false, 'message' => 'This advertisement is not available'];
          $this->response($output, 422); die;
        }
    ...
    if($check_auth_client == true){ 
      ...
      foreach ($result as $value) {  
        ...
        foreach ($coll_array as $array) {
          ...
          if($array == 'service') { 
            $Query = array('status'=>1, 'end_date'=> array('$gt'=> $todaysdate), 'category'=>$value['_id']);
          }
          else{
            $Query = array('status'=>1, 'category'=>$value['_id']);
          }   
          $categoryCount = $this->connection->community->$array->find($Query);
