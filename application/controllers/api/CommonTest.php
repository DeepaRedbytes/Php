...
class Common extends REST_Controller {
  ...
  public function check_auth_client(){
    ...
    if($check_auth_client == true){   
      ...
      //$m = new MongoClient;
$c = $this->connection->community->service;
...
// ])
  ...
  // $some = $this->connection->community->service->aggregate([
  //   {$match : {status : 1}}
  // ]);
  // print_r($some); die; 
        ...
        //array('date' => array('$gt' => $start, '$lte' => $end),'lid' => $lid);
    ...
    //     $this->connection->community->event->aggregate([{$match : {status : 1}},
    //     {$lookup: {from: "payment",localField: "_id",foreignField: "typeId",as: "payment"}},
    //     {$project : {payment : { $filter : {input : "$posts"  , as : "post", cond : { $eq : ['$$post.via' , 'facebook'] } } },
    //             admin : 1
